...
public class Renderer {
    ...
    public static void draw(Canvas canvas, Stroke[] strokes, float scale, float offset_x, float offset_y, int width1, int color1, int width2, int color2) {
        draw(canvas, strokes, scale, offset_x, offset_y, width1, color1, width2, color2, Stroke.STROKE_TYPE_PEN);
    }
     ...
     * @param strokeType Type of stroke.
     */
    private static void draw(Canvas canvas, Stroke[] strokes, float scale, float offset_x, float offset_y, int width1, int color1, int width2, int color2, int strokeType) {
        setPressureRecalculatorFactorsForF110();

        for (Stroke s : strokes) {
            int length = s.getDots().size();
