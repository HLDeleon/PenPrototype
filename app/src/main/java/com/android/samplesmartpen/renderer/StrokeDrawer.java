...
import static kr.neolab.sdk.ink.structure.Stroke.STROKE_TYPE_HIGHLIGHT;
import static kr.neolab.sdk.ink.structure.Stroke.STROKE_TYPE_NORMAL;
import static kr.neolab.sdk.ink.structure.Stroke.STROKE_TYPE_PEN;
...
//			fp -= getPressureFactor( mP[k+nAvrCnt] ) * getDistancePressureAt(k);
    ...
    protected void drawStroke(Canvas canvas, float scale, float offset_x, float offset_y, boolean bDebugMode, int type) {
        if (bDebugMode) {
            ...
            if (type == STROKE_TYPE_NORMAL)
                tempPaint = mFountainPaint;
            else if (type == STROKE_TYPE_PEN)
                tempPaint = mPencilPaint;
            else if (type == STROKE_TYPE_HIGHLIGHT)
    ...
    public void drawWriteStroke(Canvas canvas, float scale, float offset_x, float offset_y, Paint oPaint, int alphaStrokeWidth) {
        ...
        for (int k = 0; k < mN; k++) {
            if (k == 0) {
                ...
                if (k == mN - 1)
                    path.lineTo(mX[k - 1] * scale + offset_x, mY[k - 1] * scale + offset_y);
            }
        }
